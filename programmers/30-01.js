...
function solution(board, moves) {
  ...
  //       const num = board[j][moves[i] - 1];
  //       if (num !== 0) {
  //         if (num === out[out.length - 1]) {
  //           answer += 2;
  //           out.pop();
  //         } else {
  //           out.push(num);
  ...
  //       if (doll !== 0) {
  //         // 4. 방금 뽑은 인형의 위치를 빈칸으로 만들어준다.
  //         board[j][moves[i] - 1] = 0;
  //         console.log(bucket, doll);
  //         // 5-1. 바구니에 넣으려는 인형과 바구니 맨 위 인형이 동일하다면, 인형 제거 후 크레인 멈춘다.
  //         if (doll === bucket[bucket.length - 1]) {
  //           //   bucket.pop();
  //           //   bucket.splice(bucket.length - 1, 1);
  //           bucket = bucket.slice(0, bucket.length - 1);
  //           answer += 2;
  //           break;
  //         }
  //         // 5-2. 동일하지 않다면 바구니에 담고, 크레인을 멈춘다.
  //         bucket.push(doll);
  //         break;
  //       }
  //     }
  ...
  const bucket = [];
  moves.forEach((move) => {
    let pick = false; // 반복문을 더이상 실행시키지 않게 하기 위한 변수 값.
